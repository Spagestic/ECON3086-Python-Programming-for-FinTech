...
#   > ['AMGN']
def exclude_tickers(tickers: list) -> list:
    # do something here
...
def exclude_blacklisted_tickers(tickers: list, blacklisted: list) -> list:
    result = []
    for ticker in tickers:
...
def create_10_multiples_list(n: int) -> list[int]:
    multiples = []
...
def count_income_above_threshold(incomes: list[float]) -> int:
    count = 0
...
#   > 3100.0
def sum_income_above_threshold(incomes: list[float]) -> float:
    total = 0.0
    for income in incomes:
        if income > 1000:
            total += income
...
#   > 2
def count_items_above_average(numbers: list[float]) -> int:
...
#   > [0.0, 0.0]
def calculate_daily_percentage_return(prices: list[float]) -> list[float]:
    returns = []
...
#   > [500.0]
def calculate_monthly_net_income(incomes: list[float], expenses: list[float]) -> list[float]:
    net_incomes = []
    for i in range(len(incomes)):
        net_income = incomes[i] - expenses[i]
